                ...
                <div class="table-responsive">
                    <table id="myTable" class="table  table-bordered">
                        ...
                        @foreach($courses as $key => $course)
                            <tr>
                                <td><img src="{{ convertStorage($course->media->source) }}" alt=""></td>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $course->name }}</td>
                                <td>{{ $course->slug }}</td>
                                ...
                                <td>
                                    ...
                                    <a href="{{route('courses.destroy',['course' => $course->id])}}"
                                       class="btn btn-warning delete-course">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Bạn chắc chắn muốn xóa khóa học chứ ?
                </div>
...
@push('js')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            const modal = new bootstrap.Modal(document.querySelector('#courseModal'));
            const deleteCourseElement = $('.delete-course');
            const acceptDeleteCourseElement = $('.accept');
            let courseDeletePicked = null;
            ...
            $(document).on('click','.delete-course',function(event) {
                ...
                modal.show();
                courseDeletePicked = $(this).attr('href') || $(this).closest('.delete-course').attr('href');
            })

            acceptDeleteCourseElement.on('click', function () {
                if (!courseDeletePicked) {
                    modal.close();
                    return;
                };


                axios.delete(courseDeletePicked)
